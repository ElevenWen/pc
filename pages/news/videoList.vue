<template>
  <div id="newsSearch">
    ...
    <div class="container">
      <!-- 内容 -->
      <div class="content">
        <div class="news-h2">“热门视频”相关新闻</div>
        <div class="newsList" v-if="videoList.length">
          <ul>
            <li class="list-item" v-for="item in videoList" :key="item.Id" :class="{'video': item.NewsType}" @click="toDetail(item.Id)">
            ...
            <div class="el-pagination is-background" v-if="CountPage" v-show="false">
              <ul class="el-pager">
                <li class="number" v-for="i in pages" :key="i" :class="{'active': i === pageIndex}">
                  ...
                  <a :href="'/news/videoList?page=' + i" class="redirect" v-else>{{i}}</a>
                </li>
              </ul>
            </div>
            <no-ssr>
      ...
      <div class="sidebar">
          ...
          <!-- <div class="hot-video" v-if="videoList.length">
            <p class="sidebar-title">热门视频<span class="more" @click="tagSearch('热门视频')">更多&gt;</span></p>
            <ul>
              <li class="video-item" v-for="item in videoList" :key="item.id" @click="toDetail(item.Id)">
                <img :src="item.ThumbImgURL" alt="" class="video-cover">
...
export default {
  ...
  async asyncData({ store, query, error, redirect }) {
    ...
    if (videoList) {
      getStatus = 200;
      // 限制最大页数
      var CountPage = videoList ? videoList.data.data.recordCount : 0,
        pages = Math.ceil(CountPage / 8) || 1;
      if (page > pages) {
        return redirect("/news/videoList?page=" + pages);
      }
    } else {
    ...
    return {
      ...
      meta: [
        ...
        },
        {
          hid: "description",
          name: "description",
          content: "在微企宝找到了" + this.CountPage + "条“热门视频”的相关新闻"
        }
      ]
    };
  },
