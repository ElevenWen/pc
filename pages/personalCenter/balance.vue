          ...
          <div class="right_frame">
	    		<div class="balance_top">
	    			<div class="top1" id="top1">
	    				<h1>我的可用余额</h1>
	    				<p class="num">{{balance?balance:0}}</p>
	    			</div>
	    			<div class="top2">
	    				<h4>我的其它资产</h4>
	    				<p>
						...
						<p>
	    					...
	    					个
	    					<nuxt-link to="/personalCenter/currency">管理</nuxt-link>
	    				</p>
	    				<!-- <p>我的银行卡：<span>0</span>张<a href="">管理</a> </p> -->
	    			</div>
	    			<div class="top3" @click="czBtn">
	    				<div class="buttom_charge">充值/提现</div>
	    			</div>
	    		</div>
						...
						<div class="balance_container">
							<ul>
								<li v-for="(item,index) in TransactionDetail" :key="item.Id">
									...
									<span>
										{{item.Type==0?'+'+item.Amount:-item.Amount}}
									</span>
									<span class="state_span">
										{{item.Status=='1'?'交易成功':'交易中'}}
									</span>
								</li>
							</ul>
						</div>
...
export default {
	...
	methods:{
		//弹出模态框
		czBtn(){
			this.$router.push("/productDetails/83ec5bf4-8f5a-40de-a92d-43ee5d6c655e/0");
...
//				console.log(err)
		...
		GetTransactionDetail(type){
			if(type == "0"){ //收入
				...
				getData.getBalanceRecord(params).then((res) => {
