			...
			<div class="all_order_form">
				<div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
				    <ul class="layui-tab-title">
						<nuxt-link to="/personalCenter/allOrder" tag="li">全部订单</nuxt-link>
			    		...
			    		<nuxt-link to="/personalCenter/waitEvaluate" tag="li" class="layui-this">待评价<span class="layui_awaitComment" v-if="waitCommend">{{waitCommend}}</span></nuxt-link>
				    </ul>
									...
									<table class="commodity_list">
										...
										<tbody class="color_DD">
											<tr>
												...
												<td>
													<span class="color_FF immediately" v-if="item.OrderStatus=='待付款'" @click="payNow(item.OrderNumber)">立即付款</span>
													<span v-if="item.OrderStatus=='待付款'" @click="showCancelModel(this,$event,item.Id)">取消订单</span>
													...
													<div v-if="item.OrderStatus=='待评价'" v-for="(itemss,index) in item.OrderDetails" :key="itemss.Id">
														<span class="greyBorder immediately" @click="waitEvaluate(itemss.ProductIdd,itemss.type,item.Id,itemss.ProductId)">评价</span>
														<span><a href="#" v-if="item.OrderStatus=='待评价'" @click="buyAgain(item.Id)">再次购买</a></span>
													</div>
												</td>
											</tr>
										</tbody>
...
export default {
  ...
  methods:{
	  ...
	  getWaitOrder(){
		...
		getData.allOrder(params).then(res=>{
			//console.log('发发发发发发',params.type,res.data)
			this.allOrderListArr = res.data.list
	...
	//当前页
	  ...
	  //立刻付款去支付页
	//   payNow(id){
	// 	  this.$router.push({path:'/cart/payment',query:{orderId:id}})
	//   },
	  //再次购买-立刻购买
	  buyAgain(id){
	  var params = {
	  ...
	  getData.GetOrderListById(params).then(res=>{
		...
		//   arr[0].IsChecked = true;
		  var obj = {
		    ...
		    // console.log('再次购买成功111111111111111',obj)
